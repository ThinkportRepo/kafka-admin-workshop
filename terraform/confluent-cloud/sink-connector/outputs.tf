output "resource-ids" {
  value = <<-EOT
  Environment ID:   ${confluent_environment.customer.id}
  ...
  $ confluent kafka topic produce ${confluent_kafka_topic.orders.topic_name} --environment ${confluent_environment.customer.id} --cluster ${confluent_kafka_cluster.basic.id} --api-key "${confluent_api_key.app-producer-kafka-api-key.id}" --api-secret "${confluent_api_key.app-producer-kafka-api-key.secret}"
  ...
  $ confluent kafka topic consume ${confluent_kafka_topic.orders.topic_name} --from-beginning --environment ${confluent_environment.customer.id} --cluster ${confluent_kafka_cluster.basic.id} --api-key "${confluent_api_key.app-consumer-kafka-api-key.id}" --api-secret "${confluent_api_key.app-consumer-kafka-api-key.secret}"
